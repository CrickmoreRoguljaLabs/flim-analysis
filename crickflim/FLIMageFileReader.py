...
class FileReader:
            ...
            for flim_each in flim1:
                ...
                if self.ImageFormat == 'ChTime_YX':
                    ...
                    for i in range(0, self.nChannels):
                        ...
                        if self.n_time[i] > 0:
                            imageC.append(
                                np.reshape(flim_each[0, offset: offset2], (self.height, self.width, self.n_time[i]),
                                           'C'))
                        else:
                            imageC.append(np.zeros(1))  # If not acquired, it return 0 value.
                        offset = offset2
    ...
    def LoadFLIMFromMemory(self, page, fastZpage, channel):
        if self.pageValid(page, fastZpage, channel):
            if not self.FastZStack:
                fastZpage = 0
            self.FLIM3D = np.reshape(self.image[page][fastZpage][channel].astype(np.double),
                                     (self.height, self.width, self.n_time[channel]))
    ...
    def pageValid(self, page=0, fastZpage=0, channel=0):
        ...
        all_valid = self.flim and fastZValid and channel < self.nChannels and channel >= 0 and len(
            self.image) > page and len(self.image) > 0
        return all_valid
    ...
    def calculatePage(self, page=0, fastZpage=0, channel=0, lifetimeRange=[0, 64], intensityLimit=[0, 20],
                      lifetimeLimit=[1.6, 2.0], lifetimeOffset=0.5):
        if self.pageValid(page, fastZpage, channel):
